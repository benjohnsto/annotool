	    ...
	    new OpenSeadragon.MouseTracker({
			...
			*/	                
			...
			//console.log(app.manifests[app.current.manifest]);
	                ...
	                var preview_item = "<div id='" + id + "' class='preview-item active-item' data-service='" + app.outputs.service + "' data-canvas='" + app.outputs.canvas + "' data-manifest='" + manifest_url + "'>\
		    <div>" + app.outputs.html + "</div>\
		    <div class='selectcrop copyable' style='position:absolute;top:0px;left:0px;z-index:-100'>\
		    <a href='" + mirador_link + "' title='"+alttext+"' target='_blank'>" + app.outputs.html + "</a>\
		    </div>\
	...
	jQuery(document).on("click", ".gallery-item", function(e) {
	    ...
	    app.current.manifest = jQuery(this).attr('data-manifest');
	    app.current.canvas = jQuery(this).attr('data-canvas');
	    app.current.service = jQuery(this).attr('data-service');
	    app.current.version = jQuery(this).attr('data-version');
